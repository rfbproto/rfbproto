...
extended `PointerEvent`_ message.
...
`PointerEvent`_ message, the Back and Forward buttons must be treated as
not being pressed.

The bits in *extended-buttons-mask* represent the following buttons
(see [#]_ for additional context):
...
..  [#] Historically, there has been confusion regarding which  mouse
        buttons are meant to represent back and forward. Typically,
        these buttons are located on the side of a mouse and are used to
        navigate back and forward in applications such as web browsers
        and file explorers.
